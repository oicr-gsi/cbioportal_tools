...
def generate_meta_type(meta_config: Config.Config, study_config: Config.Config, verb):
    ...
    if not meta_config.datatype == 'CANCER_TYPE':
    ...
    if meta_config.datatype in ['SEG', 'GISTIC_2']:
        for field, entry in zip(ref_gene_id_zip, meta_info_map[meta_config.alterationtype + ":" + meta_config.datatype]):
            output_set.append((field, entry))
        if meta_config.datatype == 'SEG':
            try:
    ...
    if meta_config.datatype not in clinical_type and meta_config.datatype not in ['SEG']:
    ...
    output_set.append(('data_filename', 'data_{}.txt'.format(config2name_map[meta_config.alterationtype + ":" + meta_config.datatype])))
    ...
    f_out = os.path.join(study_config.config_map['output_folder'],
                         'meta_{}.txt'.format(config2name_map[meta_config.alterationtype + ":" + meta_config.datatype]))
    f = open(f_out, 'w')
