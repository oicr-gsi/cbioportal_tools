...
# TODO refactor live/dry-run tests to reduce duplication of code
...
class TestStudy(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_study.write_all(self.outDir, dry_run=False)
...
class TestStudyDryRun(unittest.TestCase):
    ...
    def setUp(self):
        ...
        #os.mkdir(self.outDir)
        ### temporary setup for output directory -- won't be deleted on test exit
        self.outDir = os.path.join('/tmp', 'CAP_expression_test_dryrun')
        from shutil import rmtree
        if os.path.exists(self.outDir):
            rmtree(self.outDir)
        os.mkdir(self.outDir)
